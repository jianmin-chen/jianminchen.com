...
Only 225 lines of code! I think it's pretty cool. There are a couple of sections. First up, I load my environment variables. There's only three of them:
...
Afterward, I have two functions, `listening_to()` and `get_posts()`, which are basically functions that get the data I'll be plugging into the `/` route.
...
-   `data`, which just contains data to be injected into HTML files.
...
If the file doesn't exist, we do the same thing, but instead of returning a `200 OK`, we return `404 Not Found`.
