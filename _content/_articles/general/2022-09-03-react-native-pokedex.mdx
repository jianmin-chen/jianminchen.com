...
date: 2022-09-03
pinned: true
title: Building a Pokedex with React Native in forty-five minutes!
---
...
Now it's time to put that stack navigator to use. Let's replace that `console.log("TODO")` in `components/PreviewCard.js`. As I mentioned before, `react-navigation` provides a `route` prop. Well, it turns out that they also provide a `navigation` prop! You can use it to push pages to the stack navigator. 
