...
Another endpoint that's pretty important to us is `https://pokeapi.co/api/v2/pokemon/${id}](https://pokeapi.co/api/v2/pokemon/`, such as [https://pokeapi.co/api/v2/pokemon/1](https://pokeapi.co/api/v2/pokemon/${id}) for Bulbasaur, who, fun fact, is the very first Pokemon in the Pokedex. This returns JSON that's something like this:
...
* NodeJS LTS release: that means, at the time of this writing, somewhere around 12 - 16. Currently am running `v16.6.0`. If you need to update, [this](https://stackoverflow.com/questions/8191459/how-do-i-update-node-js) Stack Overflow thread may be useful.
* Git, which you should already have
* [Watchman](https://facebook.github.io/watchman/docs/install#buildinstall): You only need this if you're on macOS or Windows.

We will be using Expo's command line tool to create new React Native projects. After you have these prerequisites, you can install the Expo CLI using `npm`:
...
Start a new project with `expo init pokedex`, where `pokedex` is the name of our project. Expo will return something like the following:
...
<div className="thin-media">
    ![Hello, World!](/assets/images/2022-09-03-react-native-pokedex/starter.webp)
</div>
...
(I'm not a fan of two-spaced tabs.)
...
<div className="thin-media">
    ![Huh, the text is a bit weird...](/assets/images/2022-09-03-react-native-pokedex/hello-world-broken.webp)
</div>
...
Well, at least on iOS devices, since `SafeAreaView` only supports iOS devices. No worries though - we can write our own, using inline stylesheets, combined with some components that React Native provides us:

```javascript
import { Platform, StyleSheet, Text, SafeAreaView, StatusBar, View } from "react-native";
...
<div className="thin-media">
    ![Pikachu card!](/assets/images/2022-09-03-react-native-pokedex/card.webp)
</div>

## Styling our preview cards

Okay, let's start by creating a React component. We can store it in a separate file (similar to React!), so I'll place it inside `components/PreviewCard.js`:
...
<div className="thin-media">
    ![A Pikachu preview card!](/assets/images/2022-09-03-react-native-pokedex/preview-card.webp)
</div>
...
Cool! In addition to inline stylesheets, we can use inline styling, which is something you've probably seen before if you've used NextJS or a CSS-in-JS library like [`theme-ui`](https://theme-ui.com/), for example.
...
```javascript
import { FlatList, Platform, StyleSheet, Text, SafeAreaView, StatusBar, View } from "react-native";
// ...
...
> *Sidenote*: Check out the React Native [documentation](https://reactnative.dev/docs/components-and-apis)! All the React documentation is so accessible and easy to read, and you'll definitely find it useful.
...
Now when you refresh your Expo app, you'll see ten preview cards! Refresh it another time, and you'll see ten more. *Exercise for the end: figure out how to refresh on scroll!*

<div className="thin-media">
    ![Ten new cards each time!](/assets/images/2022-09-03-react-native-pokedex/card-previews.webp)
</div>

Holy fork, that's so cool! This is pretty much the code inside the branch `milestone-1-preview-cards`. In other words, we're past our first milestone, whoo! The only exception is that we've moved some of the styles inside `components/PreviewCard.js` to a separate file - `components/cardStyles.js` for use later in other components. Here's what you'll need to move to `components/cardStyles.js`:
...
Then, inside `components/cardStyles.js`, you'll also need to import `./cardStyles` and use `cardStyles.image` and `cardStyles.title`.
...
This might not be understandable, but we're basically adding another nested function inside `useEffect`, `processListForSearch`, that will process the results of querying `https://pokeapi.co/api/v2/pokemon?limit=-1` (which returns the entire list of Pokemon, if you don't remember). We'll store each Pokemon's name and URL, for our search.
...
Okay, slow down, slow down. First of all, notice how we've updated `styles` with another "class": `input`, which contains the styles for our input container. This is used in one of the `View`s nested inside our `SafeAreaView`.
...
<div className="thin-media">
    ![Yeah, I used to be a Pokemon geek.](/assets/images/2022-09-03-react-native-pokedex/search-functionality.webp)
</div>
...
Okay, now it's time to add some navigation. There are actually different kinds, of course, such as drawer navigation (the menu opening from the left), tab navigation (bottom tab menu). We're going to be using stack navigation. A stack is a data structure when items are added to the end and items get pushed out first at the end as well. Our app will work similarly:
...
Now, we can go inside `App.js`, remove the unnecessary imports, and then add the navigation container like so:
...
Then, in our JSX, we start by wrapping everything in `NavigationContainer` and `Stack.Navigator`. Notice how we've passed the prop `initialRouteName` to `Stack.Navigator` with the value of "Home". This actually refers to the initial screen showing the name of the screen.

We've created this initial screen using `Stack.Screen`. It has, of course, the value of "Home", and is passed the component `Home`. We're also using the prop `options` - this determines the content of the top header bar. In case you have no idea what I'm referring to:

<div className="thin-media">
    ![Notice how the top bar has the name of the current screen and a "My Pokemon" link/button?](/assets/images/2022-09-03-react-native-pokedex/navigation-header.webp)
</div>
...
Down in our JSX, we're using a bunch of `View`s and `Text`s to render this information, using ternary operators to indicate information that's being loaded. A new component that we're using is `ScrollView`, which indicates a `View` that can be scrolled if necessary. Other than that, everything's been covered!

Now it's time to put that stack navigator to use. Let's replace that `console.log("TODO")` in `components/PreviewCard.js`. As I mentioned before, `react-navigation` provides a `route` prop. Well, it turns out that they also provide a `navigation` prop! You can use it to push pages to the stack navigator. Let me show you - replace the `console.log("TODO")` inside `components/PreviewCard.js` with this:
...
Oh wow, I almost forgot. We haven't actually used the `Info` screen with `Stack.Screen`, so none of this code would technically work. Let's do that now, shall we?
...
Alright! Now the page functionality should be just about working. However, if you run this right now, you'll notice that everything has a gray background. I'd like to get rid of the grayish background, which `react-navigation` has added for us. We'll just create some new "classes" inside our inline stylesheets to take care of this by making the whole background white instead of the grayish color. 
...
<div className="thin-media">
    ![There's a bunch of new Pokemon that I know nothing about.](/assets/images/2022-09-03-react-native-pokedex/info-page.webp)
</div>
...
How would we go about getting a key-value pair using `react-native-async-storage`? Here's an asynchronous function that does that:
...
We start by importing the stuff we'll need, including the functions we just wrote. We also import `useRoute` from `react-navigation`, which lets us access router parameters. We need this instead of directly using the `route` prop because the `route` prop is only passed to `Stack.Screen` components - in other words, it is only passed to screens, *not the components on each screen*. 
...
<div className="thin-media">
    ![I've already added this Pokemon to my list of favorites, so the button says "Remove", as expected.](/assets/images/2022-09-03-react-native-pokedex/remove-pokemon.webp)
</div>
...
- [ ] Learn how to eject an app using Expo!
